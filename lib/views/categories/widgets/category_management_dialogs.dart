...
Future<void> addCategory(BuildContext context, WidgetRef ref) async {
  ...
  final name = await showCategoryInput(context);
  if (name == null || !context.mounted) return;
  try {
    await ref.read(categoriesProvider.notifier).create(name: name, type: type, icon: type == 'income' ? 'work' : 'shopping_bag', color: type == 'income' ? '#22C55E' : '#EF4444');
...
Future<String?> showCategoryInput(BuildContext context) {
  final category = TextEditingController();
  return showDialog<String>(context: context, builder: (dialog) => AlertDialog(title: const Text('New category'), content: TextField(controller: category, autofocus: true, decoration: const InputDecoration(labelText: 'Category name')), actions: [TextButton(onPressed: () => Navigator.pop(dialog), child: const Text('Cancel')), FilledButton(onPressed: () { final name = category.text.trim(); if (name.isNotEmpty) Navigator.pop(dialog, name); }, child: const Text('Save'))]));
}
