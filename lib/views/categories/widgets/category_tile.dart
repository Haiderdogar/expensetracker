...
class CategoryTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Consumer(builder: (context, ref, _) {
    ...
    return Card(margin: const EdgeInsets.only(bottom: 10), child: ExpansionTile(
      ...
      children: [subcategories.when(
        loading: () => const LinearProgressIndicator(),
        error: (error, _) => Text(ErrorHandler.message(error)),
        data: (items) {
          final sorted = sortSubcategories(items, category.id, transactions);
          return Column(children: [
            ...sorted.map((item) => SubcategoryTile(subcategory: item)),
            TextButton.icon(onPressed: () => _addSubcategory(context, ref), icon: const Icon(Icons.add_rounded), label: const Text('Add subcategory')),
          ]);
        },
      )],
    ));
