...
class SubcategoryTile extends ConsumerWidget {
  ...
  Future<void> _action(
    ...
    if (action == 'edit') {
      ...
      await updateSubcategory(ref, subcategory: subcategory, name: name);
      if (context.mounted) {
        showSuccessSnackBar(context, 'Subcategory updated successfully');
      }
      return;
    }
    final ok = await confirmDelete(
      ...
      'A category must keep at least one subcategory.',
    );
    if (ok && context.mounted) {
      try {
        await deleteSubcategory(ref, subcategory);
        if (context.mounted) {
          showSuccessSnackBar(context, 'Subcategory deleted successfully');
        }
      } catch (error) {
